...
三个服务的额度采用紧凑卡片，每张卡以两个圆形进度环呈现 5 小时与每周窗口。进度方向统一表示“剩余额度”：100% 为满环，环内显示剩余百分比，旁边保留已用额度、重置时间、来源与更新时间。Settings 的手动字段同样填写剩余百分比，前端保存时转换为兼容既有 schema 的已用值。AI Workloads 标题区提供即时显示/隐藏按钮，Settings 提供同一偏好的持久开关。隐藏额度卡时 UsageCollector 不再启动新的自动刷新线程；重新显示时恢复采集并立即读取安全缓存。UsageCollector 随 `see-aicoding --web` 启动，不需要额外守护进程或单独启动命令。

### 7. Compact Dashboard 与偏好设置

Web 看板采用单页渐进披露，不再依赖多个顶层 Tab。CPU、GPU、内存、存储、网络和进程卡片固定在顶部；AI 工作负载、趋势/告警、资源排行榜、进程清单、存储和运行时区块可调整上下顺序。用户可隐藏非必要区块、切换紧凑/舒适密度和小/标准/大文字、控制是否显示空闲 AI provider，并选择进程表字段。顶部进程圆环只显示运行中进程数字，避免重复的 Run/运行后缀；程序/进程表以语义列宽对齐表头和数据，窄屏时只在表格容器内部横向滚动。

这些偏好通过 `/api/dashboard-preferences` 读写 SQLite settings，刷新页面或重启看板后仍然保留。统一设置入口同时管理中英文、五种主题、文字大小、刷新策略和本地额度值。弹窗维护独立草稿，只有点击保存才应用并持久化；重置仅准备默认草稿，点击外部空白或按 Escape 会关闭并丢弃未保存改动。后端只接受白名单 language/theme/font-size/performance/section/column id，并自动补齐新版本增加的默认值，避免旧偏好破坏后续布局。
...
紧凑 SSE 现在由 `build_snapshot(compact=True)` 直接生成，不再先构造全部进程、程序成员和完整会话后再裁剪。每个采样窗口保存同一组规范化输入；只有 `/api/snapshot` 或可见进程区需要时才延迟构造一次完整模型，紧凑与完整响应共享 `generated_at`。进程静态字段缓存 30 秒，线程数、状态和累计 CPU 时间缓存 5 秒；RSS 每轮读取一次并直接计算内存占比，macOS 不支持逐进程 I/O 时只探测一次能力。这个选择意味着进程状态和线程数在最坏情况下可比 CPU/内存实时值晚 5 秒，但避免了对数百个 PID 每 1.5–3 秒重复读取低价值字段。

完整进程详情也服从性能模式：实时 / 平衡 / 节能分别为 4.5 / 9 / 15 秒，并且只在进程区可见时运行。前端为事件和进程表维护差异签名，紧凑事件不会重复重建 80 行进程 DOM；中文模式只翻译实际重绘的区块，而不在每次 SSE 后扫描全页。节能模式关闭数值条和骨架屏装饰动画，但保留抽屉与提示等必要交互反馈。页面只使用 DOM/SVG，不建立 Canvas/WebGL 上下文。
...
- `snapshot.py` 提供 schema v3 的紧凑/完整双构建路径，同时保留旧版 `cpu_percent` 和 `disk` 别名，降低现有客户端迁移成本。
- `web.py` 在刷新窗口内缓存一次系统采样和紧凑响应，避免每个 SSE 客户端重复采集；完整进程模型与 JSON 只在首次请求时生成一次；同时对白名单化 Dashboard 偏好进行规范化，并通过 SQLite settings 持久化语言、主题、刷新策略、区块顺序、可见性、密度、额度和表格字段。
- `web_static` 只消费规范化 JSON，不包含平台判断或系统命令；资源图表引用统一 CSS 语义色，不重复定义颜色常量；视口观察器控制重区块渲染和后台轮询，差异签名避免紧凑事件重复替换大型 DOM。
