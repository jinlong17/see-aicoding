...
本轮实现覆盖 CPU、GPU、内存、交换空间、本地挂载点、磁盘 I/O、网络、传感器、全部可读取进程、程序聚合、Top 占用、进程详情，以及带安全边界的暂停/恢复/结束操作。
...
“程序”将同一应用的辅助进程聚合，适合快速判断应用总成本；“进程”保留每个 PID，支持按名称、PID、CPU、内存、GPU、磁盘 I/O 和运行时间排序。搜索覆盖名称、用户、命令、工作目录和 PID。

详情抽屉按需读取较慢或敏感度更高的字段，包括完整命令、cwd、累计 I/O、打开文件、网络连接状态和子进程。这样避免把所有详细信息塞进每次 SSE 快照。

### 3. 存储

容量卡片按挂载点显示总量、已用和剩余空间；独立趋势图显示整机读取/写入吞吐。macOS 会隐藏 Preboot、VM、Update 等内部系统卷，并把 Data 卷作为首页主数据卷。
...
all normalized data ──> snapshot schema v2 ──> cached SSE / JSON ──> four web views
...
- `telemetry.py` 是新的系统遥测边界，封装短期历史、磁盘/网络速率、传感器和 GPU 平台适配器。
- `snapshot.py` 提供 schema v2，同时保留旧版 `cpu_percent` 和 `disk` 别名，降低现有客户端迁移成本。
...
1. **阈值与事件时间线**：允许为 CPU、内存、磁盘空间、GPU 温度设置注意/警告/严重阈值，记录进入和恢复时间。
2. **异常关联**：基于本地历史识别“哪个进程启动后同时引发 CPU、磁盘和网络突增”，先用规则和 z-score，不急于引入重型 ML。
3. **持久历史与时间范围**：将内存环形缓冲区扩展为可选 SQLite 存储，提供 15 分钟、1 小时、24 小时视图和 CSV/JSON 导出。
4. **进程树与服务层**：增加父子树、launchd/systemd 服务、容器和虚拟机视图；操作权限继续独立审计。
5. **存储健康**：补充每设备吞吐、IOPS、延迟、SMART、温度和空间增长预测。
6. **网络责任主体**：在平台允许时提供逐进程连接数、远端分组和吞吐，默认隐藏完整远端地址以减少不必要的信息暴露。
7. **GPU 深度适配**：为 NVIDIA 增加 engine/encoder/decoder、ECC 和节流原因；为 AMD/Intel 补充正式库适配；Apple 逐进程 GPU 必须以明确的可用性研究为前置条件。
8. **资源策略**：在有审计日志和回滚路径后，再增加 nice/优先级、CPU affinity、资源上限和批量动作。
